...
test("renders Hero Image with a background image", () => {
  render(<HeroImage title="Hero with Image" subtitle="Image should appear" imageUrl={heroImageExample} />);
  const heroImage = screen.getByText("Hero with Image").parentElement;
  expect(heroImage).toHaveStyle(`background: url(${heroImageExample}) center/cover no-repeat`);
});

test("Hero Image should change background when disabled", () => {
  render(<HeroImage title="Disabled Hero" subtitle="This is disabled" disabled />);
